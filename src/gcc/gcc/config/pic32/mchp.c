...
static unsigned int mchp_load_resource_info(char *id, char **matched_id, const char *mchp_resource_file_in) {
    ...
    mchp_resource_file_generic =
            (char *) mchp_resource_file_in + strlen(mchp_resource_file_in) - strlen("xc32_device.info");
...
#define MCHP_XCLM64_SHA256_DIGEST_QUOTED xstr(MCHP_XCLM64_SHA256_DIGEST)
    ...
    if(require_cpp){
        return PIC32_VALID_CPP_FULL;
    }else{
        return PIC32_VALID_PRO_LICENSE;
    }
}
...
mchp_subtarget_override_options2(void) {
   ...
   */
    ...
    if (TARGET_NO_FALLBACKLICENSE &&
        ((mchp_pic32_license_valid == PIC32_VALID_PRO_LICENSE) || (mchp_pic32_license_valid == PIC32_VALID_CPP_FREE)))
      error ("Unable to find a valid license, aborting");
  }
        ...
        if (dispatch_entry->emit_dispatch != 0) {
            ...
            if ((dispatch_entry->longcall) ||
                ((mips_base_compression_flags & (MASK_MICROMIPS)) &&
                 (dispatch_entry->isr_isa_mode != pic32_isa_micromips)) ||
                (!(mips_base_compression_flags & (MASK_MICROMIPS)) &&
                 (dispatch_entry->isr_isa_mode == pic32_isa_micromips)) ||
                TARGET_LONG_CALLS) {
...
tree mchp_address_attribute(tree *decl, tree identifier ATTRIBUTE_UNUSED,
    ...
    if (TREE_CODE(*decl) == FUNCTION_DECL) {
        ...
        if ((pic32_device_mask & HAS_L1CACHE) &&
            ...
            warning(0, "Specified address of function '%s' is in the kseg1 region of an L1-cache target device",
                    attached_to);
    ...
    if (TREE_CODE(*decl) == VAR_DECL) {
        ...
        if ((pic32_device_mask & HAS_L1CACHE) &&
            ...
            warning(0, "Specified address of variable '%s' is in the kseg1 region of an L1-cache target device",
                    attached_to);
            inform(input_location,
                   "Use the 'coherent' attribute with a kseg0-region address to designate an uncached variable");
        ...
        if (TREE_VALUE(args) == get_identifier("prog")) {
            ...
            if (FUNCTION_DECL == TREE_CODE(*decl)) {
                warning(OPT_Wattributes,
                        "Ignoring space(serial_mem) attribute for function %qs. Suggest space(prog) for functions.",
                        ident);
            ...
            if (ipl_len == (sizeof("ipl7") - 1)) {
                ...
                if ((NULL != strstr(IDENTIFIER_POINTER(TREE_VALUE(ipl_tree)), "SRS")) ||
                    ...
                    if ((0 == mchp_invalid_ipl_warning) && (pic32_num_register_sets == 8)) {
                        warning(0,
                                "IPLnSOFT context saving used on a selected %qs device with 8 Shadow Register Sets, consider IPLnSRS\n",
                                mchp_processor_string);
                        mchp_invalid_ipl_warning++;
                    }
            ...
            if (SOFTWARE_CONTEXT_SAVE == current_function_type) {
                ...
                if ((interrupt_priority >= 0) || 1) {
                    ...
                    emit_insn(gen_iorsi3(gen_rtx_REG(SImode, K1_REG_NUM),
                                         gen_rtx_REG(SImode, K1_REG_NUM),
                                         GEN_INT((unsigned) interrupt_priority << SR_IPL)));
                }
                ...
                mem = gen_frame_mem(word_mode,
                                    plus_constant(word_mode, stack_pointer_rtx,
                                                  mchp_offset_srsctl));
                mips_emit_move(gen_rtx_REG(word_mode, V1_REGNUM), mem);
                emit_insn(gen_andsi3(gen_rtx_REG(SImode, V1_REGNUM), gen_rtx_REG(SImode, V1_REGNUM),
                                     GEN_INT((unsigned) 0x0F)));
                do_save_label = gen_label_rtx();
                skip_save_label = gen_label_rtx();
...
int mchp_check_for_conversion(rtx call_insn) {
    ...
    while (match) {
        ...
        if (match[1].name &&
            (strcmp(match[1].name, real_name) == 0))
            match++;
        else match = 0;
    }
    return 1;
                                ...
                                if (*s == v_flags->single_letter_equiv) {
                                    if (v_flags->single_letter_equiv == 'b') {
                                        ...
                                        if ((strncmp(name, SECTION_NAME_PBSS, 5) == 0) &&
                                            ((v_flags->flag_mask & SECTION_PERSIST) == 0))
                                            continue;
